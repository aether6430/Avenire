...
| make repo easier to change correctly after refactor | `partial` | Many shells are slimmer. Remaining pressure is now distributed across files like `particle-field.tsx`, the chat-runtime/markdown/reasoning/rolling-tool-activity clusters, and the explorer cluster instead of one giant explorer shell. |
