...
| Requirement | Status | Evidence |
| --- | --- | --- |
| reduce god files and monolithic modules where they block development | `partial` | Former hotspots like `app-sidebar.tsx`, `command-palette.tsx`, `command-palette-groups.tsx`, `dashboard-sidebar-content.tsx`, `student-calendar.tsx`, `student-calendar-desktop.tsx`, `student-calendar-desktop-surface.tsx`, `onboarding-modal-steps.tsx`, `rolling-tool-activity-surface.tsx`, `rolling-reasoning.tsx`, `markdown.tsx`, `messages.tsx`, `mermaid.tsx`, `use-chat-runtime.ts`, `use-chat-scroll.ts`, `use-explorer-file-action-operations.ts`, `use-explorer-pane-surfaces.ts`, `use-file-preview-pane-header.tsx`, `widget-renderer-model.ts`, `WidgetPrimitiveRenderer.tsx`, `settings-workspace-selected-sections.tsx`, `file-preview-markdown-pane.tsx`, `preview-attachment-content.tsx`, `explorer-browse-surface.tsx`, `multimodal-input.tsx`, `multimodal-input-surface.tsx`, `data-imports-section.tsx`, and `sidebar-files-panel.tsx` were reduced dramatically. |
...
| make repo easier to change correctly after refactor | `partial` | Many shells are slimmer. Remaining pressure is now distributed across the particle-field/model cluster, the chat-runtime/reasoning cluster, and the remaining explorer/files runtime cluster, with more of the workspace shell and files runtime now moved into explicit local model and hook/surface owners instead of one giant explorer shell. |
