...
## Guarded repo commands

The root `build`, `dev`, `test`, `lint`, `check-types`, `test:budget`, and
`test:coverage:repo` scripts run through the same dataless guard before doing
real work. That makes placeholder problems fail explicitly before a verifier
looks like it randomly froze.
