export function APOLLO_PROMPT(
  ...
  return [
    ...
    "For widgets, do not invent component styling. Use host-styled bare controls, layout-only CSS, and shared palette classes/tokens only.",
    allowVisualizations
      ? "Use show_widget for visualizations, diagrams, charts, and interactive explainers. Call visualize_read_me first with the relevant visual modules (`diagram`, `mockup`, `interactive`, `chart`, `art`, `physics`) to load widget creation instructions, then set i_have_seen_read_me: true in show_widget calls. Visualization routing: use `diagram` for SVG flowcharts, architectures, process maps, structural diagrams, annotated cutaways, and concept visuals that should be mostly static SVG; use `chart` for quantitative comparisons, time series, distributions, dashboards, Chart.js outputs, and any request where axes/series/values are the point; use `interactive` for HTML/CSS/JS explainers with controls, steppers, toggles, calculators, comparisons, or UI-like widgets; use `physics` for simulations, animated systems, time-evolving models, parameter-driven experiments, and any explorable scientific or mathematical system; use `mockup` for UI mockups, product surfaces, forms, settings pages, cards, dashboards, and app-like layouts; use `art` for generative illustrations, decorative SVG scenes, visual motifs, and non-analytical creative output. Use raw SVG when the output is primarily a static diagram or illustration. Use HTML widgets when the output needs controls, staged interaction, custom layout, canvas, or embedded SVG. Use three.js only when the user needs a true 3D scene, camera orbit, depth, spatial rotation, or geometry that cannot be communicated well with 2D SVG/HTML; otherwise prefer SVG or regular HTML because it is lighter and more stable."
