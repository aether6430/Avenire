...
import {
  buildRealtimeFilesHeaders,
  resolveRealtimeFilesQuery,
  toRealtimeFilesInvalidateChunk,
} from "./realtime-files-route-model";

export async function handleRealtimeFilesRouteGet(request: Request) {
  const query = resolveRealtimeFilesQuery(request);

  if (!(query.workspaceUuid && query.token)) {
  ...
  const verification = verifyFilesRealtimeToken(
    query.token,
    query.workspaceUuid
  );
  if (!verification.ok) {
  ...
  const body = new ReadableStream<Uint8Array>({
    start: async (controller) => {
      try {
        const created = await createFilesRealtimeSubscriber(
          query.workspaceUuid
        );
      ...
      void subscriber.subscribe(channel, (rawMessage) => {
        try {
          ...
          write(toRealtimeFilesInvalidateChunk(payload));
        } catch {
  ...
  return new Response(body, {
    headers: buildRealtimeFilesHeaders(),
  });
}
