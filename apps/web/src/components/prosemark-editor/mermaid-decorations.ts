...
class MermaidWidget extends WidgetType {
  ...
  constructor(source: string, editMode: boolean) {
    super();
    this.source = source;
...
function findEnclosingFencedCode(view: EditorView, host: HTMLElement) {
  ...
  for (const side of [-1, 1] as const) {
    ...
    while (node.name !== "FencedCode" && node.parent) {
      node = node.parent;
    }
...
function toggleEditMode(
  ...
  const sel = computeToggleSelection(
    editMode,
    fence.from,
    fence.to,
...
const mermaidFoldExtension = foldableSyntaxFacet.of({
  ...
  buildDecorations: (state, node, selectionTouchesRange) => {
    ...
    if (!parsed.info.trim().toLowerCase().startsWith("mermaid")) {
      return undefined;
    }
...
const mermaidTheme = EditorView.baseTheme({
  ...
  ".cm-mermaid-canvas-stage svg[data-xychart-colors]": {
    "--xychart-color-1":
      "color-mix(in srgb, var(--accent) 45%, var(--fg-base) 55%)",
    "--xychart-color-2":
      "color-mix(in srgb, var(--accent) 20%, var(--fg-base) 80%)",
    "--xychart-color-3":
      "color-mix(in srgb, var(--accent) 8%, var(--fg-base) 92%)",
    "--xychart-color-4":
      "color-mix(in srgb, var(--accent) 4%, var(--fg-base) 96%)",
    "--xychart-color-5":
      "color-mix(in srgb, var(--accent) 2%, var(--fg-base) 98%)",
  ...
  ".cm-mermaid-canvas-edit, .cm-mermaid-canvas-zoom-btn": {
    ...
    opacity: "0",
    transition:
      "opacity 120ms ease-out, background-color 120ms ease-out, color 120ms ease-out",
  },
