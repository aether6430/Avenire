...
export const toggleBold: StateCommand = inlineWrapCommand(
  "**",
  "StrongEmphasis"
);
export const toggleItalic: StateCommand = inlineWrapCommand("*", "Emphasis");
export const toggleInlineCode: StateCommand = inlineWrapCommand(
  "`",
  "InlineCode"
);
...
export const clearInlineFormatting: StateCommand = ({ state, dispatch }) => {
  ...
  dispatch(
    state.update(changes, { userEvent: "input.format.clearFormatting" })
  );
  return true;
};
...
const formattingKeymap: KeyBinding[] = Object.values(formattingCommands).map(
  (c) => ({
    key: c.chord,
    run: c.run,
  })
...
export const markdownFormatting: Extension = Prec.high(
