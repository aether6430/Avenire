...
import { foldableSyntaxFacet, foldExtension } from "@prosemark/core";
...
class HtmlBlockWidget extends WidgetType {
  ...
  constructor(sanitizedHtml: string, rawText: string) {
    super();
...
function isInteractiveHtmlTarget(target: EventTarget | null) {
  return (
    ...
    target.closest(
      "a,button,input,select,textarea,summary,video,audio,label,[role='button']"
    ) !== null
  );
}
...
const htmlBlockSelectOnMouseDown = EditorView.domEventHandlers({
  mousedown(event, view) {
    ...
    if (!target.closest(".cm-html-block-widget")) {
      return false;
    }
    if (isInteractiveHtmlTarget(target)) {
      return false;
    }
...
const selfClosingBlockParser: BlockParser = {
  ...
  parse(cx: BlockContext, line: Line) {
    ...
    if (!/^\s*<[a-z][\w-]*(?:\s+[^>]*)?\s*\/\s*>\s*$/i.test(line.text)) {
      return false;
    }
...
const detailsBlockParser: BlockParser = {
  ...
  before: "HTMLBlock",
  parse(cx: BlockContext, line: Line) {
    if (!/^\s*<details(?:\s|>|$)/i.test(line.text)) {
      return false;
    }
    const from = cx.lineStart + line.pos;
