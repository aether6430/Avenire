...
export interface MindsetCardItem {
  back: ReactNode;
  front: ReactNode;
  id: string;
...
interface MindsetCardStackProps {
  autoAdvanceMs?: number;
  cards: MindsetCardItem[];
  className?: string;
  flipped?: boolean;
  onFlippedChange?: (next: boolean) => void;
  onOrderChange?: (nextCards: MindsetCardItem[]) => void;
  showCounter?: boolean;
...
export function MindsetCardStack({
  autoAdvanceMs,
  cards,
  className,
  flipped,
      ...
      {showStackLabel ? (
        <div className="mb-3 flex items-center justify-between gap-3 px-1">
          <div className="space-y-0.5">
            <p className="font-medium text-foreground text-sm">{stackLabel}</p>
