...
import { WidgetPrimitiveChart } from "@/components/widget-primitive-render-chart";
...
export function renderWidgetPrimitiveNode(node: WidgetSpecNode, key: string) {
  switch (node.type) {
    case "stack":
      return (
        <div
        ...
        >
          {node.children.map((child, index) =>
            renderWidgetPrimitiveNode(child, `${key}-${index}`)
          )}
        </div>
      );
    case "grid":
    ...
    case "chart":
      return <WidgetPrimitiveChart key={key} node={node} />;
    case "progress":
      return (
...
export function WidgetPrimitiveRenderContent({ spec }: { spec: WidgetSpec }) {
  return (
    <div className="w-full rounded-lg bg-card p-3 text-card-foreground">
      ...
      {renderWidgetPrimitiveNode(spec.root, "root")}
    </div>
  );
}
