...
function hasNativeAssetExtension(pathname, kind) {
	return (
		NATIVE_ASSET_EXTENSIONS[kind]?.some((extension) =>
...
function inferNativeAssetKind(value) {
	...
	if (
		...
		hasNativeAssetExtension(pathname, "pdf")
	) {
		return "pdf";
	}
	if (
		mimeType.startsWith("image/") ||
		hasNativeAssetExtension(pathname, "image")
	) {
		return "image";
	}
	if (
		mimeType.startsWith("video/") ||
		hasNativeAssetExtension(pathname, "video")
	) {
		return "video";
	}
	if (
		mimeType.startsWith("audio/") ||
		hasNativeAssetExtension(pathname, "audio")
	) {
		return "audio";
	}
...
export function buildClipMarkdown(context, noteTitle) {
	...
	const lines = [
		`# ${title}`,
		"",
		`Source: [${context.page.title}](${context.page.url})`,
	];
