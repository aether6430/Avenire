...
const els = {
	...
	defaultDestinationSelect: document.getElementById(
	...
	includeCaptureProperties: document.getElementById(
		"include-capture-properties",
	),
...
function renderSettings() {
	...
	els.includeSourceProperties.checked = Boolean(
		state.settings?.includeSourceProperties,
	);
	els.includeCaptureProperties.checked = Boolean(
		state.settings?.includeCaptureProperties,
	);
}
...
async function checkSession() {
	try {
		...
		renderFolderBrowser();
		setResult(
			"Unable to reach Avenire. Check the app origin and that the web app is running.",
			"error",
		);
		return false;
	}
...
async function loadWorkspaces() {
	...
	if (
		!state.workspaces.some((entry) => entry.workspaceId === state.workspaceId)
	) {
...
async function loadFolders(workspaceId, parentId) {
	...
	state.selectedFolderId =
		state.selectedFolderId ?? payload.currentFolder?.id ?? null;
	renderFolderBrowser();
}

function syncEditorFromSelectedDestination() {
	const selected = state.destinations.find(
		(entry) => entry.id === state.selectedDestinationId,
	);
	if (!selected) {
...
async function createDestination() {
	...
	if (!response.ok) {
		const message = await readErrorMessage(
			response,
			"Unable to create the destination preset.",
		);
...
async function updateDestination() {
	...
	if (!response.ok) {
		const message = await readErrorMessage(
			response,
			"Unable to update the destination preset.",
		);
		setResult(message, "error");
		return;
	}
...
async function deleteDestination() {
	...
	if (!response.ok) {
		const message = await readErrorMessage(
			response,
			"Unable to delete the destination preset.",
		);
		setResult(message, "error");
		return;
	}
