...
async function readNativeAssetFile(nativeAsset) {
	...
	const contentType =
		response.headers.get("content-type")?.trim() || nativeAsset.mimeType;
...
async function createUploadSession(nativeFile, destination) {
	...
	if (!response.ok) {
		throw new Error(
			await readErrorMessage(response, "Unable to prepare upload."),
		);
	}
...
async function requestSessionParts(sessionId, partNumbers) {
	const response = await api(
		state.appOrigin,
		`/api/uploads/sessions/${sessionId}/parts`,
		{
	...
	if (!response.ok) {
		throw new Error(
			await readErrorMessage(response, "Unable to prepare upload parts."),
		);
	}
...
async function uploadSessionPartsConcurrently(
	parts,
	nativeFile,
	partSizeBytes,
) {
...
async function completeUploadSession(
	...
	if (!response.ok) {
		throw new Error(
			await readErrorMessage(response, "Unable to finalize upload."),
		);
	}
...
function renderAuth() {
	if (state.user) {
		...
		els.authStatus.textContent =
			"Sign in to Avenire before you clip anything from this page.";
	}
...
async function checkSession() {
	try {
		...
		renderAuth();
		setResult(
			"Unable to reach Avenire. Check the extension settings and that the web app is running.",
			"error",
		);
		return false;
	}
...
async function loadDestinations() {
	try {
		...
		await storageSet({
			[SELECTED_DESTINATION_KEY]: state.selectedDestinationId,
		});
		renderDestinationOptions();
	} catch {
...
async function refreshContext(resetDraft = true) {
	try {
		...
		setResult(
			error?.message ??
				"Unable to communicate with the active tab. Try a normal web page.",
			"error",
		);
	}
}
...
async function clipNow() {
	...
	if (state.sourceContext.page.nativeAsset) {
		const nativeFile = await readNativeAssetFile(
			state.sourceContext.page.nativeAsset,
		);
		...
		if (parts.length !== partNumbers.length) {
			setResult(
				"The upload session did not return the expected parts.",
				"error",
			);
			return;
		}
...
async function initialize() {
	...
	state.lastAuthCompleteAt =
		typeof stored[AUTH_COMPLETE_KEY] === "number"
			? stored[AUTH_COMPLETE_KEY]
			: null;
	state.selectedDestinationId =
		typeof stored[SELECTED_DESTINATION_KEY] === "string"
			? stored[SELECTED_DESTINATION_KEY]
			: "";
...
els.signIn.addEventListener("click", () => {
	void sendToBackground({ type: "avenire.open-login" }).then(() => {
		setResult(
			"Login tab opened. Finish sign-in there and the clipper will update automatically.",
			"success",
		);
	});
});
...
els.addHighlight.addEventListener("click", () => {
	void sendToActiveTab({ type: "avenire.add-highlight" })
		...
		.catch((error) => {
			setResult(
				error?.message ?? "Unable to save a highlight from this page.",
				"error",
			);
		});
});
...
els.clearHighlights.addEventListener("click", () => {
	void sendToActiveTab({ type: "avenire.clear-highlights" })
		...
		.catch((error) => {
			setResult(
				error?.message ?? "Unable to clear highlights on this page.",
				"error",
			);
		});
});
