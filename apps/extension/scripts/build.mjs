...
const buildTarget =
	targetArg?.split("=", 2)[1] === "firefox" ? "firefox" : "chrome";
const distDir = join(rootDir, "dist", buildTarget);

const appOrigins = (
	process.env.AVENIRE_EXTENSION_APP_ORIGINS ??
...
async function copySource() {
	...
	await cp(
		join(webBrandingDir, "avenire-logo-mark.svg"),
		join(distDir, "avenire-logo-mark.svg"),
	);
	await cp(
		join(webBrandingDir, "avenire-logo-full.png"),
		join(distDir, "avenire-logo-full.png"),
	);
